...
def aggregate_preds_by_country(countries, resampled_dfs_by_country, pred_key, latest_dates):
    for c,country in enumerate(countries):
        ...
        if c == 0:
            ...
            # take date and preds
            df_this_key = df_this[df_this['date'] == date_this][['date', pred_key]].reset_index(drop=True)

        else:
            # append
            df_this_day = df_this[df_this['date'] == date_this][[pred_key]].reset_index(drop=True)
            df_this_key = df_this_key.merge(df_this_day, left_index=True,
                                                  right_index=True, suffixes=('',"_"+country))

    # add the first country back in as a suffix
    df_this_key = df_this_key.rename(columns={pred_key:
                    pred_key+'_{0}'.format(countries[0])})
...
def main():
    ...
    for country in countries:
        ...
        pred_no_pol = preds_this_country[preds_this_country['date'] == latest_date_this_country][pred_no_pol_key].values
        modeled_no_policy.append(pred_no_pol)

        # get predictions with policy
        pred_pol = preds_this_country[preds_this_country['date'] == latest_date_this_country][pred_pol_key].values
        modeled_with_policy.append(pred_pol)

        # get confirmed cases (with policy)
        cases_this_country = cases_dict[country]
        cases = cases_this_country[cases_this_country['date'] == latest_date_this_country]['cases'].values
    ...
    for c,country in enumerate(countries):
        ...
        print('we predict a difference of {0:,} cumulative cases due to to policy'.format(int(est_diffs[c,0])))
    ...
    print('using actual case (black dots) baseline this number would be {0:,}'.format(int(est_diffs_with_reported_cases.sum())))
    ...
    for c,country in enumerate(countries):
        print('in {0},'.format(country), end=' ')
        print('at day {0},'.format(latest_dates[c]),end=" ")
        print('we predict {0:,} cumulative cases; in total there were {1:,}'.format(int(modeled_with_policy[c,0]),
                                                                             cases_confirmed[c,0]))
    print()
    ...
    for c,country in enumerate(countries):
        print('in {0},'.format(country), end=' ')
        print('at day {0},'.format(latest_dates[c]),end=" ")
        print('we predict there would have been {0:,} had no policies been enacted'.format(int(modeled_no_policy[c,0])))

   # 4. use resampled predictions to get intervals
    df_no_pol_pred = aggregate_preds_by_country(countries, 
                                                resampled_dfs_by_country,
                                                pred_no_pol_key,
                                                latest_dates)

    df_pol_pred = aggregate_preds_by_country(countries, 
                                          resampled_dfs_by_country,
                                          pred_pol_key,
                                          latest_dates) 
    ...
    for c,country in enumerate(countries):
        print("{0:,} (95% resample range [{1:,} to {2:,}]) more cases in {3} (cumulative, on {4}), ".format(int(est_diffs[c,0]), 

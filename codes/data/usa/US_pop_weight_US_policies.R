...
us_policy_data <- readxl::read_xlsx("gpl-covid/data/raw/usa/US_covid_policies_pre_school_closure_data.xlsx") %>% 
  rename(adm1_id = "adm1_name")


# read in the school closure data:
school_closure_data <- read_csv("gpl-covid/data/interim/usa/US_school_closures_clean.csv") %>% 
...
adm1_population_data <- read_csv("gpl-covid/data/interim/adm/adm1/adm1.csv") %>% 
...
adm2_population_data <- read_csv("gpl-covid/data/interim/adm/adm2/adm2.csv") %>% 
  filter(adm0_name == 'USA') %>% 
...
download("https://simplemaps.com/static/data/us-cities/1.6/basic/simplemaps_uscities_basicv1.6.zip", 
                                 dest="gpl-covid/data/raw/usa/us_cities_population.zip", mode="wb") 

# list of files inside master.zip
master_list <- as.character(unzip("gpl-covid/data/raw/usa/us_cities_population.zip", list = TRUE)$Name)

# load the first file "uscities.csv"
adm3_population_data <- read.csv(unz("gpl-covid/data/raw/usa/us_cities_population.zip", "uscities.csv"), header = TRUE,
                 sep = ",") %>% 
...
write.csv(pop_weighted_policy_data, "gpl-covid/data/interim/usa/US_COVID-19_policies.csv")

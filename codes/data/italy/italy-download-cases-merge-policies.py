...
def process_raw_and_interim_health():
	...
	def impute_each_cumulative_column(adm_cases, adm_id_col):
		...
		for src_col in adm_cases_cum_cols:
		    ...
		    adm_cases = cimpute.impute_cumulative_df(adm_cases, src_col, dst_col, adm_id_col)
...
def get_interim_cases():
	if reload_raw:
		...
		return load_interim_cases(path_italy_interim_region), load_interim_cases(path_italy_interim_province)

def main():
	adm1_cases, adm2_cases = get_interim_cases()
	policies = read_policies()
	adm1_cases, adm2_cases, policies = merge_health_and_policies(adm1_cases, adm2_cases, policies)

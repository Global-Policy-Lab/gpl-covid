...
fix_issues <- function(data){
  ...
  # examine_issues(data, cum_confirmed_cases)
  while(do_issues_exist(data, cum_recoveries_imputed)){
    warning("Fixing an issue with cum_recoveries")
    browser() # After filtering to >= 10 there were no issues. Will revisit if they turn up.
    data <- fix_first_issue(data, cum_recoveries_imputed, impute = TRUE)
    data <- fix_first_issue(data, cum_recoveries, impute = FALSE)
  }
  while(do_issues_exist(data, cum_confirmed_cases_imputed)){
    browser() # After filtering to >= 10 there were no issues. Will revisit if they turn up.
    warning("Fixing an issue with cum_confirmed_cases")
    data <- fix_first_issue(data, cum_confirmed_cases)
  }
  while(do_issues_exist(data, cum_confirmed_cases_imputed)){
    browser() # After filtering to >= 10 there were no issues. Will revisit if they turn up.
    data <- fix_first_issue(data, cum_deaths)
  }
  data %>% 

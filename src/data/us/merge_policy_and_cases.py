...
def download_and_process_policy_csv():
	...
	policy_data['travel_countries'] = policy_data_raw['travel_ban_country_list'].copy()
	...
	policy_keys_nonbinary = ['travel_countries', 'no_gathering_size']
	...
	aggregation_styles['travel_countries'] = general_union
	...
	for i in range(len(policies_affecting_all_states)):
	    ...
	    for policy_key in policy_keys:
	        ...
	        if policy_key == 'travel_ban_intl_out':
	            ...
	            def add_countries(x):
	                return np.union1d(x,this_policy['travel_countries'])
	            
	            cases_data.loc[affected_case_rows,'travel_countries'] = cases_data.loc[affected_case_rows,'travel_countries'].apply(add_countries)

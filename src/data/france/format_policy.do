...
import delim "raw/france/france_policy_static_20200316.csv", clear 
drop if minor == 1
...
merge m:1 adm2 using "interim/france/departement_info.dta", keep(3) nogen
*ad hoc for corse
replace pop = 327283 if adm1 == 94 & adm2 == 2
drop departement_name adm1_name cheflieu superficie densite region_id
sort adm1 date

collapse (sum) running_var pop, by(date adm0 adm1 policy_cat)
...
merge m:1 adm1 using "interim/france/region_ID.dta", nogen keep(1 3)
...
import delim "interim/france/france_confirmed_cases_by_region.csv", clear
...
"no_gathering_national_5000" "no_gathering_national_1000" ///
...
"social_distance_size" "social_distance_national" {
	replace `var' = 0 if `var' == .
...
*drop oversea counties for now
drop if adm1 < 10
*save
format date %tdCCYY-NN-DD

outsheet * using "processed/adm1/FRA_processed.csv", replace comma

...
# 
# Sensitivity of the final analysis to these two options will 
# be evaluated.
# 
...
# Set paths
base_dir <- 'C:/Users/Andy Hultgren/Documents/ARE/GPL/coronavirus_sprint'
cases_in <- paste0(base_dir, '/intermediate/covid_iran_cases.csv')
policies_in <- paste0(base_dir, '/intermediate/covid_iran_policies.csv')
file_out <- paste0(base_dir, '/cleaned/IRN_processed.csv')
file_out_adm0 <- paste0(base_dir, '/cleaned/adm0/IRN_processed.csv')
...
cases.data <- cases.data %>%
  ...
  mutate( cumulative_confirmed_cases = cumsum(tmp) ) %>%
  mutate( cumulative_deaths = cumsum(new_deaths_national)) %>%
  ungroup()

# Re-insert missing values where subnational data was missing
cases.data$cumulative_confirmed_cases[ is.na(cases.data$new_confirmed_cases) ] <- NA

# Do the cumulative sum, imputing new cases at the adm2 level for the days in which it was not 
# reported (3/2/20 and 3/2/20). Save as a separate set of columns with "_with_imputations" names.
cases.data$new_confirmed_cases_with_imputations <- cases.data$new_confirmed_cases
...
for (d in missing.dates) {
  ...
  cases.data$new_confirmed_cases_with_imputations[ (as.character(cases.data$date)==d) & is.na(cases.data$new_deaths_national) ] <- round( weights * aggregate_to_distribute)
}

# Report cumulative cases and deaths, using imputed data for days with missing data
cases.data <- cases.data %>%
  group_by(adm0_name, adm1_name, adm2_name) %>%
  mutate( cumulative_confirmed_cases_with_imputations = cumsum(new_confirmed_cases_with_imputations) ) %>%
  ungroup()
...
cases.data <- cases.data[c('date', 'adm0_name', 'adm1_name', 'adm2_name', policy.types, 'new_confirmed_cases', 'cumulative_confirmed_cases', 
                           'new_confirmed_cases_with_imputations', 'cumulative_confirmed_cases_with_imputations', 
                           'new_deaths_national', 'cumulative_deaths')]

# Split out adm2 and adm0 data, and write to .csv
cases.data.adm2 <- cases.data[ !is.na(cases.data$adm2_name), 
                               c('date', 'adm0_name', 'adm1_name', 'adm2_name', policy.types, 'new_confirmed_cases', 'cumulative_confirmed_cases', 
                                 'new_confirmed_cases_with_imputations', 'cumulative_confirmed_cases_with_imputations')]

cases.data.adm0 <- cases.data[ is.na(cases.data$adm2_name), 
                               c('date', 'adm0_name', policy.types, 'new_confirmed_cases', 'cumulative_confirmed_cases', 
                                 'new_deaths_national', 'cumulative_deaths')]
